...
const parseList = payload => {
    ...
    const $return = []
    $('.item-list').find("a").each(function(index, item) {
...
const parseDetail = payload => {
    ...
    const $ = node.cheerio.load(payload.html)
    const $return = []
    $('.prd-detail').find("img").each(function(index, item) {
...
const init = async () => {
    ...
    for (let i = options.start; i < json.length; i++) {
        ...
        for (let j = 0; j < length; j++) {
            const img = imgArr[j]
            if (img.indexOf('detail.jpg') === -1 && img.indexOf('modelsize.jpg') === -1 && img.indexOf('tn-op-line.jpg') === -1 && img.indexOf('page_20.jpg') === -1 && img.indexOf('ro-logo.jpg') === -1 && img.indexOf('washing.jpg') === -1) {
                console.log(img)
